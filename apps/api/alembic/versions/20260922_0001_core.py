...
def upgrade() -> None:
    op.create_table("competitions", sa.Column("id", sa.BigInteger(), sa.Identity(), primary_key=True), sa.Column("code", sa.String(16), nullable=False), sa.Column("name", sa.String(120), nullable=False), sa.Column("country_code", sa.String(2), nullable=False), sa.UniqueConstraint("code"))
    op.create_index("ix_competitions_code", "competitions", ["code"])
    op.create_table("clubs", sa.Column("id", sa.BigInteger(), sa.Identity(), primary_key=True), sa.Column("slug", sa.String(120), nullable=False), sa.Column("name", sa.String(160), nullable=False), sa.Column("short_name", sa.String(12), nullable=False), sa.UniqueConstraint("slug"))
    op.create_index("ix_clubs_slug", "clubs", ["slug"])
    op.create_table("methodology_versions", sa.Column("id", sa.BigInteger(), sa.Identity(), primary_key=True), sa.Column("version", sa.String(64), nullable=False), sa.Column("description", sa.Text(), nullable=False), sa.Column("published_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False), sa.UniqueConstraint("version"))
    op.create_table("data_sources", sa.Column("id", sa.BigInteger(), sa.Identity(), primary_key=True), sa.Column("name", sa.String(120), nullable=False), sa.Column("status", sa.String(32), nullable=False), sa.Column("coverage", sa.Integer(), nullable=False), sa.Column("notes", sa.Text(), nullable=False), sa.CheckConstraint("coverage BETWEEN 0 AND 100", name="ck_data_sources_coverage_range"), sa.UniqueConstraint("name"))
    op.create_table("players", sa.Column("id", sa.BigInteger(), sa.Identity(), primary_key=True), sa.Column("source_player_id", sa.String(120), nullable=False), sa.Column("name", sa.String(160), nullable=False), sa.Column("club_id", sa.BigInteger(), sa.ForeignKey("clubs.id", ondelete="RESTRICT"), nullable=False), sa.Column("birth_year", sa.Integer(), nullable=False), sa.UniqueConstraint("source_player_id"))
