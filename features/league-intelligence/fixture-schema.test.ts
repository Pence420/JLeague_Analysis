...
describe("league fixture schema", () => {
  it("rejects duplicate team ids", () => {
    const duplicate = { ...sampleLeagueDataset, teams: [...sampleLeagueDataset.teams, sampleLeagueDataset.teams[0]] };
  ...
  it("rejects coverage outside zero to one hundred", () => {
    const invalid = {
      ...sampleLeagueDataset,
      teams: sampleLeagueDataset.teams.map((team, index) => index === 0 ? { ...team, coverage: 101 } : team),
    };
  ...
  it("accepts null for an unavailable analytical metric", () => {
    const valid = {
      ...sampleLeagueDataset,
      teams: sampleLeagueDataset.teams.map((team, index) => index === 0 ? { ...team, expectedGoals: null } : team),
    };
