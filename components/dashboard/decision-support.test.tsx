...
import { buildLeagueDashboard, DEFAULT_AXES } from "@/features/league-intelligence/analytics";
...
const viewModel = buildLeagueDashboard(sampleLeagueDataset, { axes: DEFAULT_AXES, minimumMinutes: 900 });
...
describe("decision-support modules", () => {
  ...
  it("shows reasons, risk, coverage, and confidence for recruitment signals", () => {
    ...
    expect(screen.getByText(viewModel.recruitmentSignals[0].risk)).toBeVisible();
    expect(screen.getByText(/coverage/i)).toBeVisible();
    expect(screen.getByText(new RegExp(`${viewModel.recruitmentSignals[0].confidence} confidence`, "i"))).toBeVisible();
  });

  it("renders exactly three ranked analyst findings", () => {
    const { container } = render(<AnalystBrief findings={viewModel.analystBrief} />);
